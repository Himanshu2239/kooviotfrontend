...
const FgRecheckingRejectionTable = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const res = await axios.get('https://kooviot.vercel.app/packing/fgRecheckingRejection/fetch', {
    ...
    return (
        <div className="px-4 md:px-10 py-6">
            <h2 className="text-2xl text-yellow-600 font-bold mb-6">FG Rechecking Rejection Report</h2>
            ...
            <div className="overflow-x-auto h-[65vh] border shadow-md rounded-lg mt-6">
                <table className="min-w-full text-sm text-left text-gray-700 border-collapse">
                    <thead className="bg-yellow-200 text-xs uppercase text-gray-600 sticky top-0 z-10">
                        <tr>
                            <th className="px-4 py-2" rowSpan="2">Date</th>
                            <th className="px-4 py-2 text-center" colSpan="5">Items</th>
                    ...
                    <tbody>
                        {currentItems.length === 0 ? (
                            <tr>
                                <td colSpan={6} className="text-center py-10 text-gray-500 font-semibold text-lg">
                                    Result not found
                                </td>
                            </tr>
                        ) : (
            ...
            <div className='flex md:flex-row flex-col justify-end md:space-x-72 space-x-24'>
                ...
                <div className="mt-4 font-semibold text-lg text-right">
                    Total FG Rechecking Rejection: {totalRejection}
                </div>
            </div>
        </div>
    );
